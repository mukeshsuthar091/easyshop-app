...
import { error_response, pagination, paginationResponse, response } from "../interfaces";
...
export class ResponseHandler {
    ...
    PaginationResponse(Res: Response, status_code: number, message: string, data: any, total: number, p: pagination): Response<any, Record<string, any>> {
        const response: paginationResponse = {
            status_code, message, page: p.page, limit: p.limit, total, data, status: 1,
        }
    ...
    GenerateError(status_code: number, code: any, message: string, error: any) {
